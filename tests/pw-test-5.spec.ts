...
test('test', async ({ page }) => {
  ...
  await expect(page.getByRole('article')).toContainText('You can chain methods that create a locator, like page.getByText() or locator.getByRole(), to narrow down the search to a particular part of the page.');
  await expect(page.getByRole('heading', { name: 'Matching inside a' })).toBeVisible();
